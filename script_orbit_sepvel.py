'''
the purpose of this file was to generate the scripts for just a restricted
amount of orbit parameters and the separation velocities. which leaves out
the time and date ranges and reduces the radius Periapsis and eccentricity
ranges in order run a detailed analysis in a few days.
'''
...
radiusPeriapsis = list(float_range(6728.137, 6978.137, 62.5))  # kilometers
eccentricityRange = list(float_range(0.0, 0.3, 0.1))
...
angleFromVelAxis = [0, 60*(pi/180), 120*(pi/180), 180*(pi/180), 240*(pi/180), 300*(pi/180)]
